...
exports.loadTokenFile = loadTokenFile;
exports.saveTokenFile = saveTokenFile;
exports.refreshAccessToken = refreshAccessToken;
exports.ensureProjectId = ensureProjectId;
...
function loadTokenFile(tokenPath = (0, token_paths_1.getDefaultTokenPath)()) {
    if (!fs_1.default.existsSync(tokenPath)) {
        throw new Error(`token 文件不存在: ${tokenPath}\n请先运行 extract-token`);
    }
    let raw;
    try {
        raw = JSON.parse(fs_1.default.readFileSync(tokenPath, 'utf8'));
    }
    catch (e) {
        throw new Error(`token 文件 JSON 解析失败: ${tokenPath}: ${e.message}`);
    }
...
/** 写 token 文件，强制 0600 */
function saveTokenFile(tf, tokenPath = (0, token_paths_1.getDefaultTokenPath)()) {
    (0, token_paths_1.ensureConfigDir)(tokenPath);
    fs_1.default.writeFileSync(tokenPath, `${JSON.stringify(tf, null, 2)}\n`, { encoding: 'utf8', mode: 0o600 });
...
async function refreshAccessToken(entry) {
    ...
    if (!res.ok || !data.access_token) {
        throw new Error(`OAuth refresh 失败 (HTTP ${res.status}): ${data.error || ''} ${data.error_description || text.slice(0, 200)}`);
    }
 ...
 * 取不到时错误消息只含响应键名，不含值。
 */
async function ensureProjectId(entry) {
    if (entry.projectId)
        return entry.projectId;
    const url = `${config_1.default.antigravity.baseUrl}/v1internal:loadCodeAssist`;
    const payload = JSON.stringify({
        metadata: {
            ideType: 'ANTIGRAVITY',
            ideVersion: config_1.default.antigravity.ideVersion,
            platform: config_1.default.antigravity.platformEnum,
        },
    });
    ...
    // 一律 Content-Length。
    const res = await (0, antigravity_client_1.postStream)(url, entry.accessToken, payload, undefined, false);
    const text = await new Promise((resolve) => {
        let s = '';
        const body = (0, antigravity_client_1.decodeBody)(res);
        body.on('data', (c) => (s += c.toString('utf8')));
    ...
    if (status < 200 || status >= 300) {
        ...
        const err = Object.assign(new Error(`loadCodeAssist 失败 (HTTP ${status})，响应键: [${keys}]`), { status });
        throw err;
    }
    ...
    const candidates = [
        ...
        json.project,
        json.projectId,
        json.response?.cloudaicompanionProject,
        json.response?.project,
        json.response?.projectId,
    ];
    ...
    const respKeys = json.response && typeof json.response === 'object'
        ? Object.keys(json.response).map((k) => `response.${k}`)
        : [];
    throw new Error(`loadCodeAssist 未返回 projectId，响应键: [${[...topKeys, ...respKeys].join(', ')}]`);
}
/** 写回 token 文件，保留同名条目的其余字段 */
function writeBack(next, name, tokenPath) {
    try {
        const tf = loadTokenFile(tokenPath);
        const idx = tf.tokens.findIndex((t) => t.name === name);
        if (idx >= 0)
            tf.tokens[idx] = { ...tf.tokens[idx], ...next };
        else
            tf.tokens.push(next);
 ...
 * 只重试 1 次。
 */
async function withAuth(entry, fn, tokenPath = (0, token_paths_1.getDefaultTokenPath)()) {
    try {
        ...
        if (!hadPid)
            writeBack({ ...entry, projectId }, entry.name, tokenPath);
