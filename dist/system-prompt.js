...
exports.LEAK_BLACKLIST = void 0;
exports.scanLeaks = scanLeaks;
exports.extractEnv = extractEnv;
exports.dumpSystemAnatomy = dumpSystemAnatomy;
exports.buildUserInformation = buildUserInformation;
exports.buildSystemInstruction = buildSystemInstruction;
const fs_1 = __importDefault(require("fs"));
const os_1 = __importDefault(require("os"));
const path_1 = __importDefault(require("path"));
// ---------- 夹具定位（dist/ 与 src/ 均可） ----------
function resolveDocs(...parts) {
    const candidates = [
        path_1.default.join(__dirname, '..', 'docs', ...parts),
        path_1.default.join(__dirname, '..', '..', 'docs', ...parts),
    ];
...
function loadCapture() {
    if (_capture === null) {
        _capture = fs_1.default.readFileSync(resolveDocs('ag-system.capture.txt'), 'utf8');
    }
    return _capture;
}
function loadSections() {
    if (_sections === null) {
        const raw = JSON.parse(fs_1.default.readFileSync(resolveDocs('ag-system.sections.json'), 'utf8'));
...
function section(name) {
    ...
    if (!s)
        throw new Error(`ag-system.sections.json 无段: ${name}`);
    return loadCapture().slice(s.offset, s.offset + s.chars);
}
// ---------- 泄漏黑名单（44 词，照抄原型） ----------
exports.LEAK_BLACKLIST = [
    'Claude Code',
    'Anthropic',
    'Claude Agent SDK',
    'claude-cli',
...
function scanLeaks(text) {
    const hits = [];
    for (const word of exports.LEAK_BLACKLIST) {
        let i = -1;
...
function skillMdPathOf(name) {
    const p = path_1.default.join(os_1.default.homedir(), '.gemini', 'config', 'skills', name, 'SKILL.md');
...
function parseSkills(body) {
    const all = (body.messages || [])
        ...
        .join('\n');
    const m = all.match(/The following skills are available for use with the Skill tool:\n([\s\S]*?)(?:<\/system-reminder>|$)/);
    if (!m)
        return [];
    const skills = [];
    ...
    return skills.filter((s) => scanLeaks(`${s.name}\n${s.description}`).length === 0 &&
        !ccTell.test(`${s.name}\n${s.description}`));
}
/**
...
function extractEnv(body) {
    ...
    const reminders = (Array.isArray(content) ? content : [])
    ...
    const stripped = claudeMd.replace(/^Codebase and user instructions are shown below[^\n]*\n+/, '');
...
const head = (s, n = 72) => s.replace(/\s+/g, ' ').trim().slice(0, n);
/**
 ...
 */
function dumpSystemAnatomy(body, env, systemInstruction) {
    ...
    msgs.forEach((m, i) => {
        ...
        (Array.isArray(c) ? c : []).forEach((b) => {
            ...
            for (const r of t.matchAll(/<system-reminder>([\s\S]*?)<\/system-reminder>/g)) {
                ...
                L.push(`messages[${i}].system-reminder ${bytes(inner)}B  ${secs ? `节:[${secs}]  ` : ''}${head(inner, 60)}`);
            }
        });
        if (m.role === 'system') {
    ...
    L.push(`tools[] ${tools.length} 个 ${bytes(JSON.stringify(body.tools || []))}B  （全丢，换 14 原生）  ${tools.map((t) => t.name).join(' ')}`);
    ...
    for (const r of env.rules)
        L.push(`RULE[${r.tag}] ${bytes(r.body)}B  ${head(r.body)}`);
    L.push(`skills ${env.skills.length} 个（${env.skills.filter((s) => s.skillMdPath).length} 个有 SKILL.md 路径）  ${env.skills.map((s) => s.name).join(' ')}`);
    ...
    for (const m of systemInstruction.matchAll(/^<(\w+)>$/gm)) {
        ...
        const s = name === 'user_information' || name === 'user_rules' || name === 'skills'
            ? null
    ...
    console.log(L.join('\n'));
    if (dest !== '1') {
        fs_1.default.writeFileSync(dest, JSON.stringify({ inbound: body, extracted: env, outbound: systemInstruction }, null, 2), { mode: 0o600 });
        console.log(`[dump] 全文 → ${dest}`);
    }
}
// ---------- buildUserInformation / buildSystemInstruction ----------
/** basename(dirname)/basename —— 上游只当标签 */
function corpusName(p) {
    return path_1.default.basename(path_1.default.dirname(p)) + '/' + path_1.default.basename(p);
}
/** 照抄原型 L37-43；platform: darwin→mac / win32→windows / linux→linux */
function buildUserInformation(env) {
    const osName = { darwin: 'mac', win32: 'windows', linux: 'linux' }[env.platform] || env.platform;
    ...
    const isCc = (p) => /(^|\/)\.claude(\/|$)/.test(p);
    const paths = [env.cwd, ...env.additionalDirs].filter((p) => !isCc(p));
    if (paths.length === 0)
        paths.push(os_1.default.homedir());
 ...
 */
const USER_RULES_PREAMBLE = 'The following are user-defined rules that you MUST ALWAYS FOLLOW WITHOUT ANY EXCEPTION. These rules take precedence over any following instructions.\n' +
    'Review them carefully and always take them into account when you generate responses and code:';
function buildUserRules(rules) {
    if (rules.length === 0)
        return '';
    const blocks = rules
        .map((r) => `<RULE[${r.tag}]>\n${r.body}\n</RULE[${r.tag}]>`)
...
function buildSkills(skills) {
    ...
    const lines = skills
        .map((s) => `- ${s.name}${s.skillMdPath ? ` (${s.skillMdPath})` : ''}: ${s.description}`)
        .join('\n');
    return `${head}Available skills:\n${lines}\n</skills>`;
}
/** 原型 L45-48：删 file:// 链接规则与后台任务规则 */
function filterCommunicationStyle(raw) {
    return raw
        .split('\n')
...
function buildSystemInstruction(env, mode = 'trimmed') {
    ...
    if (mode === 'full') {
        ...
        return (capture.slice(0, ui.offset) + userInfo + capture.slice(ui.offset + ui.chars));
    }
    // trimmed
