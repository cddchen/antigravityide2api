...
exports.AntigravityError = void 0;
exports.buildEnvelope = buildEnvelope;
exports.newSessionId = newSessionId;
exports.parseSseLine = parseSseLine;
exports.isSseDoneLine = isSseDoneLine;
exports.createStreamAcc = createStreamAcc;
exports.accumulateFrame = accumulateFrame;
...
class AntigravityError extends Error {
    constructor(message, status, upstreamStatus, body) {
        super(message);
...
function buildEnvelope(opts) {
    return {
        ...
        request: {
            ...
            generationConfig: {
                maxOutputTokens: config_1.default.antigravity.maxOutputTokens,
                thinkingConfig: {
...
function newSessionId() {
    const buf = crypto_1.default.randomBytes(8);
...
/** 是否为 data:[DONE] 终止行 */
function isSseDoneLine(line) {
    const raw = line.endsWith('\r') ? line.slice(0, -1) : line;
    if (!raw.startsWith('data:'))
        return false;
    return raw.slice(5).trimStart() === '[DONE]';
}
function createStreamAcc() {
    return {
        text: '',
        thoughtText: '',
        fcParts: [],
 ...
 */
function accumulateFrame(acc, frame, cbs) {
    if (frame.error) {
 ...
 */
function postStream(url, accessToken, bodyJson, signal, 
/**
...
chunked = true) {
    ...
    const mod = u.protocol === 'http:' ? http_1.default : https_1.default;
    return new Promise((resolve, reject) => {
        const req = mod.request({
            ...
            headers: {
                Host: u.host,
                'User-Agent': config_1.default.antigravity.userAgent,
                ...(chunked
...
function decodeBody(res) {
    if ((res.headers['content-encoding'] || '').toLowerCase() === 'gzip') {
        ...
        return res.pipe(require('zlib').createGunzip());
    }
    return res;
}
/**
...
async function streamGenerate(opts) {
    ...
    const url = `${config_1.default.antigravity.baseUrl}/v1internal:streamGenerateContent?alt=sse`;
    // 超时 + 调用方 signal 合并
    const signals = [AbortSignal.timeout(config_1.default.antigravity.requestTimeout)];
    if (opts.signal)
    ...
    const cbs = { onText: opts.onText, onThought: opts.onThought };
        ...
        stream.on('data', (chunk) => {
            ...
            while ((nl = buffer.indexOf('\n')) !== -1) {
                ...
                try {
                    ...
                    reject(new AntigravityError(`SSE JSON parse failed: ${e instanceof Error ? e.message : String(e)}`, 502, undefined, line.slice(0, 300)));
                    return;
                }
                if (!frame)
                    continue;
...
if (require.main === module) {
    ...
    assert(/^agent\/[^/]+\/\d+\/[^/]+\/\d+$/.test(env.requestId), `requestId shape: ${env.requestId}`);
    // 2. systemInstruction.role === 'user'
    assert(env.request.systemInstruction.role === 'user', 'systemInstruction.role');
    // 3. toolConfig VALIDATED
    assert(env.request.toolConfig.functionCallingConfig.mode === 'VALIDATED', 'toolConfig mode');
    ...
    assert(acc.fcParts[0].thoughtSignature === 'EuULCuILARFNMg/sig-must-keep', 'thoughtSignature preserved');
    ...
    const parsed = parseSseLine('data: {"response":{"candidates":[{"finishReason":null}]},"traceId":"abc"}');
    ...
    try {
        ...
        assert(e.status === 400, 'error status 400');
        assert(/thoughtSignature 丢失/.test(e.message), 'thoughtSignature hint');
    }
    assert(threw, 'frame.error must throw');
    // 静默 exit 0
    process.exit(0);
}
//# sourceMappingURL=antigravity-client.js.map